...
#program using the odeint integration to solve Newton's gravitational equations for one massive body
...
# particles evenly spaced on rings, takes argumen R_num (number of rings)
...
#function which sets up differential equation to be solved for gravity
#takes arguments t(time) and y(array containing velocities and positions)
def Acceleration(y, t):
    pos_vel_step_prime = []
    N = int(len(y)/6)
    for i in range(N):
        pos_vel_step_list = [y[6*i+3], y[6*i+4], y[6*i+5] , -y[6*i]*G_const*Mass_1/(y[6*i]**2+y[6*i+1]**2)**(3/2), -y[6*i+1]*G_const*Mass_1/(y[6*i]**2+y[6*i+1]**2)**(3/2), y[6*i+2]]
        pos_vel_step_prime.append(pos_vel_step_list)
    pos_vel_step = np.array(pos_vel_step_prime).reshape(len(y))
    return pos_vel_step
...
def Single_Galaxy(T_max, N_steps, R_num):
    r_0 = Init_Positions(R_num)
    v_0 = Init_Speed(r_0)

    pos_vel_0_prime = np.concatenate([r_0.T, v_0.T], axis = 1) # combined position and velocity array
    pos_vel_0 = pos_vel_0_prime.reshape(6*len(pos_vel_0_prime[:,0]))

    t = np.linspace(0, T_max, N_steps)
    r_pos_prime = integrate.odeint(Acceleration,pos_vel_0, t)
    r_pos = r_pos_prime.reshape(N_steps,120,6)
    r_pos_galaxy = np.zeros_like(r_pos)

    R_pos = np.append(r_pos, r_pos_galaxy, axis = 1)
...
#test function to see if a circular orbit for a single mass is formed
#takes arguments T_max (max time) and N_steps (number of steps)
def Single_Body(T_max, N_steps):
    r_0 = [3,0,0]
    v_0 = Init_Speed(r_0)
    ...
    t = np.linspace(0, T_max, N_steps)
    R_pos = integrate.odeint(Acceleration, pos_vel_0, t)
...
#takes arguments T_max (max time) and N_steps (number of steps)
def Energy(T_max, N_steps):
    R_pos = Single_Body(T_max,N_steps)
    r_pos = R_pos[:,:3]
    velocity = R_pos[:,3:6]
...
#takes arguments i, x, y, scat, d_t, time and Acc_fac (acceleration factor)
...
#takes arguments T_max, N_steps, Acc_fac, R_num
def Simulation(T_max, N_steps, Acc_fac, R_num):
    R_pos = Single_Galaxy(T_max, N_steps , R_num)
    ...
    #ani.save('SingleModelodeint001.mp4', writer = writer)
