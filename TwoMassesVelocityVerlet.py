...
#takes arguments r_pos (the current particle position) and r_galaxies (galaxy positions)
...
def Velocity_Step(r_pos_1, r_pos, velocity, r_galaxies_1, r_galaxies, d_t):
    return velocity - 0.5*d_t*(Acceleration(r_pos,r_galaxies)+Acceleration(r_pos_1,r_galaxies_1))
...
def Two_Galaxies(N, N_snapshot, d_t, R_num):
    ...
    for i in range(0,N-1):
        ...
        velocity[i+1,:,:] = Velocity_Step(r_pos[i+1,:,:], r_pos[i,:,:], velocity[i,:,:], r_pos_galaxies[i+1,:,:], r_pos_galaxies[i,:,:], d_t)
        v_galaxies[i+1,:,:] = Velocity_Step(r_pos_galaxies[i+1,:,:], r_pos_galaxies[i,:,:], v_galaxies[i,:,:], r_pos_galaxies[i+1,:,:], r_pos_galaxies[i,:,:], d_t)
...
def Simulation(N_steps, N_snapshot, d_t, Acc_Fac, R_num):
    ...
    #ani.save('TidalTailsVVerletrev.mp4', writer = writer)
    
    plt.show()

#run simulation
#takes arguments N_steps, N_snapshot(N value for which snapshot is printed) d_t, Acc_Facc(the acceleration factor for the movie) and R_num (number of rings)
Simulation(100000, 90000, 0.001, 10, 5)
