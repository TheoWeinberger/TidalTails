...
#program using the verlet stormer method of integration to solve Newton's gravitational equations for two massive bodies

G_const = 1.0  # gravitational constant
Mass_1 = 1.0 # mass of central heavy mass
Mass_2 = 3.0 # mass of second heavy mass
...
def Position_Step(r_pos_n, r_pos_n_1, r_galaxies, d_t):
    return 2*r_pos_n - r_pos_n_1  - Acceleration(r_pos_n, r_galaxies)*(d_t)**2
...
def Two_Galaxies(N, N_snapshot, d_t, R_num):
    ...
    #initialise
    r_pos[0,:,:] = r_0
    r_pos[1,:,:] = r_0 + v_0*d_t - 0.5*Acceleration(r_0,r_0_galaxies)*(d_t)**2
    r_pos_galaxies[0,:,:] = r_0_galaxies
    r_pos_galaxies[1,:,:] = r_0_galaxies + v_0_galaxies*d_t - 0.5*Acceleration(r_pos_galaxies[0,:,:],r_0_galaxies)*(d_t)**2

    #apply timestepping
    for i in range(1,N-1):
        r_pos[i+1,:,:] = Position_Step(r_pos[i,:,:], r_pos[i-1,:,:], r_pos_galaxies[i,:,:], d_t)
        r_pos_galaxies[i+1,:,:] = Position_Step(r_pos_galaxies[i,:,:], r_pos_galaxies[i-1,:,:], r_pos_galaxies[i,:,:], d_t)
...
#run simulation
#takes arguments N_steps, N_snapshot(N value for which snapshot is printed) d_t, Acc_Facc(the acceleration factor for the movie) and R_num (number of rings)
Simulation(400000, 90000, 0.001, 1000, 5)
